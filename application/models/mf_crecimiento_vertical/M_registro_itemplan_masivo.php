...
class M_registro_itemplan_masivo extends CI_Model {
	...
	function registrarItemplanMasivoCV($dataInsert,$dataInsertDetalle, $dataInsertSeguimiento) {
		...
		try{
			...
			if($this->db->affected_rows() != 1) {
				...
				$this->db->insert('planobra_detalle_cv', $dataInsertDetalle);
				if($this->db->affected_rows() != 1) {
					$this->db->trans_rollback();
					throw new Exception('Hubo un error al registrar en la tabla planobra_detalle_cv');
				}else{
	...
	function crearSolCreacionForItemplanDiseno($dataPlanobra, $solicitud_oc_creacion, $item_x_sol){
        $data['error'] = EXIT_ERROR;
        $data['msj']   = null;
        try{
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                $this->db->insert('solicitud_orden_compra_diseno', $solicitud_oc_creacion);
                if($this->db->affected_rows() != 1) {
                    $this->db->trans_rollback();
                    throw new Exception('Error al insertar en solicitud_orden_compra');
                }else{
                    $this->db->insert('itemplan_x_solicitud_oc_diseno', $item_x_sol);
                    if($this->db->affected_rows() != 1) {
                        $this->db->trans_rollback();
                        throw new Exception('Error al insertar en itemplan_x_solicitud_oc');
                    }else{
