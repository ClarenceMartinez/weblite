...
class M_bandeja_solicitud_oc extends CI_Model {
	...
	//	log_message('error', print_r($arrayUpdatePlanObra,true));
        ...
        if($this->db->affected_rows() <= 0) {
            ...
            $this->db->update_batch('itemplan_x_solicitud_oc', $arrayUpdateSolicitudxItem, 'codigo_solicitud_oc');
	...
	function updatePOByItemplan($codigo_solicitud, $estado_actual, $estado_final) {
		$sql = "UPDATE planobra_po ppo,
				 ...
				 WHERE ppo.estado_po = ?
                   ...
                   AND CASE WHEN s.idProyecto = 52 THEN ppo.idEstacion NOT IN (1) ELSE true END
	...
	function actualizarSolicitud($codigo_solicitud, $arrayData){
	    ...
	    try{
	        ...
	        $this->db->update('solicitud_orden_compra', $arrayData);
	        if($this->db->affected_rows() <= 0) {
	            throw new Exception('Error al actualizar en la tabla solicitud_orden_compra!!');
	        }else{
