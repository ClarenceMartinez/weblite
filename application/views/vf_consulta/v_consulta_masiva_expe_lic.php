...
<head>
        <html lang="en"  class="root-text-sm"> 
        <title>
            PangeaCo | CONSULTA DE EVIDENCIAS LICENCIA
        </title>  
    ...
    <body class="<?php echo ESTILO_BODY ?>">
                            ...
                            <div class="col-xl-12">
                                <div id="panel-1" class="panel">
                                    <div class="panel-hdr form-group">
                                        <h2>
                                                CONSULTA MASIVA EXPEDIENTES LICENCIA
                                        </h2>
                                        ...
                                        <div class="panel-content">             
                                            ...
                                            <div class="row" style="-20px;">     <!--                                                     
                                                ...
                                                <div class="col-lg-8 col-md-8 col-sm-8 col-8">                                                            
                                                    <div class="form-group">
                                                        <label class="form-label" for="txt_nom_pro">Lista Itemplan ( ingresar los itemplan separados por "," (coma) ejemplo: 18-0110900080,18-0110900081,18-0110900083
</label>
                                                ...
                                                <div class="col-lg-2 col-md-2 col-sm-2 col-4">
                                                    <a style="/*margin-left: 15px;*/ margin-top: 55px;/* height: 50%;*/" class="btn btn-md btn-outline-primary waves-effect waves-themed searchT">
                ...
                <div class="modal-content" style="/*overflow: scroll;*/">
                    ...
                    <div class="modal-body">
                        <div class="settings-panel">
                                ...
                                <div id="contTablaPartidasAll" class="form-group">
                                </div>
        ...
        <script>
            ...
            $('.searchT').on('click', function() {
                var txt_itemplan_list        = $.trim($('#example-textarea').val().replace(/\s+/g,''));
                num_exp_down = 0;
                console.log(txt_itemplan_list);
              //  location.href = 'expedientes_lic_download?' + 'dat=' + cod_obra;
             
                $.ajax({
                        'type'  :   'post',
                        'url'   :   'findMasExpeLic',
                        'data'  :   {txt_itemplan_list       : txt_itemplan_list},
                        'async' :   false
                }).done(function (data){
                    var data = JSON.parse(data);
                    if (data.error == 0) {
                        reInitTableLightMasExpeLic(data.json_bandeja);         
                        num_exp_down = data.num_val_down;       
                    }else{
            ...
            function reInitTableLightMasExpeLic(ejemplo_json){		 
                ...
                table = $('#dt-basic-example').DataTable(
                        {   
                            ...
                            buttons: [   
                                ...
                                className: 'btn-outline-danger exportEvi',
                                titleAttr: 'Descargar Destalle Stock',
                                action: function ( e, dt, node, config ) {       
                                    if(num_exp_down > 0){
                                        $('.exportEvi').attr("disabled", true);
                                        var txt_itemplan_list        = $.trim($('#example-textarea').val().replace(/\s+/g,'')); 
                                        location.href = 'expedientes_lic_download?' + 'dat=' + txt_itemplan_list;
                                    }else{
                                        mostrarNotificacion(1,'warning','No se pudo realizar la accion','No hay expedientes para descargar!');
                                    }
                                    /*
                                    $('body').loading({
        ...
        function openModalDetallePartidasAll(component) {
                var itemplan = $(component).attr('data-itemplan');
                $.ajax({
                    type: 'POST',
                    'url': 'getDetPartidasAll',
                    data: {
                        cod_obra: itemplan
                    },
                    'async': false
                    ...
                    var data = JSON.parse(data);
                    if (data.error == 0) {   
                        $('#contTablaPartidasAll').html(data.html);
                        initExistDataTableLight('tbPartidasAll');
                        modal('modalDetallePO');                      
                    } else {
