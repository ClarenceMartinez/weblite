...
<head>
    ...
    <body class="<?php echo ESTILO_BODY ?>">
                                            ...
                                            <div class="row" style="-20px;">                                              
                                                ...
                                                <div class="col-lg-2 col-md-2 col-sm-2 col-6">  
                                                    ...
                                                    <select class="select2" id="selectEstado" aria-label="usertype">
                                                        ...
                                                        <option value="0">PENDIENTE COTIZAR</option>
        ...
        <script>
            ...
            function generarIp(btn) {
                var cod_cluster = btn.data('cod');
                var eecc        = btn.data('eecc');
                ...
                Swal.fire({        
                    title: 'Generar Itemplan',
                    icon: 'warning',
                    html: '<div class="row form-group">'+
                                    '<div class="col-md-12">'+
                                        '<label style="color: red;font-weight: 700;" id="adver_txt"></label>'+
                                        '<label>Seleccione código de Inversión</label>'+
                    ...
                    confirmButtonText: 'Si, Generar Itemplan!',
                    cancelButtonClass: 'btn btn-secondary',
                    allowOutsideClick: false,
                    onOpen: function () {
                        $.ajax({
                                type    :   'POST',
                                'url'   :   'getInversionComb',
                                data    :   { eecc  : eecc,
                                              cod   : cod_cluster},
                                'async' :   false
                        }).done(function(data){
                            data = JSON.parse(data);
                            console.log('exede:'+data.exede_sla);
                            if(data.exede_sla   ==  1){//SI EXEDIO
                                $('#adver_txt').html('La cotizacion supero los 60 dias en estado "Aprobado" ¿desea generar El itemplan?');
                            }else{
                                $('#adver_txt').html('');
                            }
                        ...
                        if (result.value) {
                            ...
                            var cod_inversion = $('#cmbInversion option:selected').val();
                            formData.append('cod_inversion', cod_inversion);//APROBAR
                            $.ajax({
                                type  :	'POST',
                                url   :	'genIpFromB2b',
                                data  :	formData,
                                ...
                                if(data.error == 0){
                                    mostrarNotificacion(2, 'success', 'Se genero el Codigo de obra:'+data.itemplan, 'Verificar');
                                }else{
            ...
            function rechazarCoti(btn) {
                ...
                Swal.fire({        
                    ...
                    html: 'Esta seguro de rechazar la cotizacion?',
                    icon: 'warning',
                    html: '<div class="form-group">'+
                            ...
                            '</div>',
                    showCancelButton: true,        
                    confirmButtonText: 'Si, rechazar Cotizacion!',
                    cancelButtonClass: 'btn btn-secondary',
                    allowOutsideClick: false
                }).then((result) => {
                        if (result.value) {
                            ...
                            var formData = new FormData();
                            formData.append('codigo', cod_cluster);
                            formData.append('estado', 2);//RECHAZAR
