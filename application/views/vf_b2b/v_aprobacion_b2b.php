    ...
    <body class="<?php echo ESTILO_BODY ?>">
                                            ...
                                            <div class="row" style="-20px;">                                              
                                                ...
                                                <div class="col-lg-2 col-md-2 col-sm-2 col-6">  
                                                    ...
                                                    <select class="select2" id="selectEstado" aria-label="usertype">
                                                        ...
                                                        <option selected value="1">PDT APROBAR</option>
        ...
        <script>
            ...
            function devolverCoti(btn) {
                ...
                Swal.fire({        
                    title: 'Devolver Cotizacion',
                    html: 'Esta seguro de devolver la cotizacion?',
                    icon: 'warning',          
                    html: '<div class="form-group">'+
                                '<textarea class="col-md-12 form-control" placeholder="Ingresar Comentario..." style="height:80px;background:#F9F8CF" id="comentarioText"></textarea>'+
                            '</div>',          
                    showCancelButton: true,        
                    confirmButtonText: 'Si, devolver Cotizacion!',
                    cancelButtonClass: 'btn btn-secondary',
                    allowOutsideClick: false
                }).then((result) => {
                        if (result.value) {
                            ...
                            var formData = new FormData();
                            formData.append('codigo', cod_cluster);
                            formData.append('estado', 3);//DEVOLVER A PENDIENTE          
                            var comentario = $('#comentarioText').val();
                            formData.append('comentario', comentario);                 
                            $.ajax({
                                type  :	'POST',
                                ...
                                var data = JSON.parse(data);                                
                                 if(data.error == 0){
                                    mostrarNotificacion(2, 'success', 'Se devolvio la cotizacion correctamente', 'Verificar');
                                }else{
                                    mostrarNotificacion(1, 'error', data.msj, 'Verificar');
                                }
