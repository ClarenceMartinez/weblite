...
class C_aprobacion_b2b extends CI_Controller {
    ...
    public function index() {
	    ...
	    if($idUsuario != null){           
            ...
            $result = $this->lib_utils->getHTMLPermisos($permisos, 42, null, 45, null);
            $data['opciones'] = $result['html'];
            $data['header'] = $this->lib_utils->getHeader();
            $iddEECC = $this->session->userdata('idEmpresaColabSesion');		 
            $data['json_bandeja'] = $this->getArrayPoBaCoti($this->m_cotizacion_b2b->getAllCotizaciones(1,null,null,$iddEECC));
            $this->load->view('vf_b2b/v_aprobacion_b2b',$data);        	  
    	 }else{
    ...
    public function getArrayPoBaCoti($listaCotiB2b){
        ...
        if($listaCotiB2b!=null){
            foreach($listaCotiB2b as $poMat){ 
                ...
                if($poMat['estado'] ==  1){
                    $btnValidar = ' <a class="btn btn-sm btn-outline-primary btn-icon btn-inline-block mr-1" title="Validar Cotizacion" 
                                        aria-expanded="true" data-cod="'.$poMat['codigo_cluster'].'"
                                        onclick="aprobarCoti($(this));"><i class="fal fa-check"></i>
                                    </a>';
                    ...
                    $btnDeolver = ' <a class="btn btn-sm btn-outline-primary btn-icon btn-inline-block mr-1" title="Devolver Cotizacion" 
                                        aria-expanded="true" data-cod="'.$poMat['codigo_cluster'].'"
                                        onclick="devolverCoti($(this));"><i class="fal fa-arrow-left"></i>
                                    </a>';
                }else{
                    $btnValidar     = '';
                    $btnRechazar    = '';
                ...
                $actions    = $btnRegCot. $detalleCoti. $btnDescarga. $btnDeolver.$btnValidar. $btnRechazar; 
                array_push($listaFinal, array($actions,
                    $poMat['codigo_cluster'],$poMat['sisego'], $poMat['nombre_estudio'], $poMat['subproyectoDesc'], $poMat['empresaColabDesc'],$poMat['codigo'],$poMat['distrito'],$poMat['tipo_enlace']
                   ,$poMat['costo_materiales'],$poMat['costo_mano_obra'],$poMat['clasificacion'],$poMat['fecha_registro'],$poMat['estadoDesc']));
            }     
        }                                                            
        return $listaFinal;
    }
