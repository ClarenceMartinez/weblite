...
class C_matriz_seguimiento extends CI_Controller {
    ...
    public function getInfoCableNodo()
    {
        ...
        try {
            ...
            if (count($lista)> 0)
            {
                ...
                {
                    array_push($divicauList, $item['divicau']);
                }
                // $divicauList = sort($divicauList, 0);
            }
            $data['lista']  = $lista;
            $data['total']  = count($lista);
            $data['divicau']= $divicauList;
        } catch (Exception $e) {
         $data['msj'] = $e->getMessage();
        }
        echo json_encode($data);
    ...
    public function saveMatSegInfoPIN()
    {
        ...
        $info  = $this->m_matriz_seguimiento->getInfoCableNodoV2(trim($nodo), trim($cable), trim($divicau));
        ...
        $data_old = array(
            ...
            if (trim($divicau) == "")
            {
                // $this->m_matriz_seguimiento->actualizaMatrizSeguimientoPIN1($arrayUpdate, $cable, $nodo);
                if (count($info)> 0)
                {   
                    ...
                    $item = $info[0];
                    $this->m_matriz_seguimiento->actualizaMatrizSeguimientoPIN2($arrayUpdate, $item['id']);
                }
            }
            else
            {
                ...
                $invoque1 = $this->m_matriz_seguimiento->actualizaMatrizSeguimientoPIN1V2($arrayUpdate, $info[0]);
            }
