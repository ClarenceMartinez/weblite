...
class C_panel extends CI_Controller {
    ...
    public function index() {
	    ...
	    if($idUsuario != null){
            ...
            $data['opciones'] = null;
            $data['cardModulo'] = $this->getModulosCards();
            $this->load->view('v_panel',$data);        	  
    	 }else{
			 $idUsuario  = $this->session->userdata('idUsuarioGlobal');
			 if($idUsuario != null){
				$resultado = $this->m_login->getUserInfo(null, $idUsuario);
				$perfilesInfo = $this->m_login->getPerfilesDescByIdUsuario($idUsuario);
    ...
    function getModulosCards() {
     //   $arrayDataModulo = $this->m_utils->getModulos();
     $arrayDataModulo = array();
     $html = null;
        foreach($arrayDataModulo as $row) {
            $html .= '<div class="card" style="text-align: center;">
                        <div class="w-100 bg-fusion-50 rounded-top" style="width: 50%!important; margin:0px auto;margin-top: 1em;">
                            <img src="'.RUTA_IMG_INTERNAS.$row['rutaImagen'].'" class="card-img-top" alt="..."  style="border-radius: 10em;" >
                        </div>
                        <div class="card-body">
                            <h5 class="card-title"><strong>'.$row['nomModulo'].'</strong></h5>
                            <p class="card-text">This is a wider card with supporting text below as a natural lead-in to additional content.</p>
                        </div>
                        <div class="card-footer">
                            <a href="'.$row['routeBienvenido'].'" class="btn btn-primary" style="color: #fff;background-color: #4cb133;border-color: #09b15c;">Ir al módulo</a>
                        </div>
                    </div>';
        }
