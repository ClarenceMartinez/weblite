...
class C_bandeja_solicitud_oc_diseno extends CI_Controller {
    ...
    public function index() {
        ...
        if($idUsuario != null){           
            ...
            $data['json_bandeja'] = $this->getArrayPoBaOC($this->m_bandeja_solicitud_oc_diseno->getSolicitudOcNew(1, null,null));//1 = SOLO PENDIENTES
            //$data['tablaSolicitudOc'] = $this->getTablaSolicitudOc(NULL, NULL, array(ID_ESTADO_PLAN_DISENIO, ID_ESTADO_PLAN_PDT_OC));
            $this->load->view('vf_orden_compra/v_bandeja_solicitud_oc_diseno',$data);            
         }else{
    ...
    function filtrarSolicitudOCDiseno() {
        $data['msj'] = null;
        ...
        $data['json_bandeja'] = [];
        $data['json_bandeja']   = $this->getArrayPoBaOC($this->m_bandeja_solicitud_oc_diseno->getSolicitudOcNew($estado , $cod_obra, $cod_soli));
    ...
    function getDataAtenderSolicitudOcDiseno() {
        $data['error']    = EXIT_ERROR;
        $data['msj']      = null;
        try {
    ...
    function getTablaAtenderSolicitudOc($codigoSolicitud) {
        $dataArrayDetalle = $this->m_bandeja_solicitud_oc_diseno->getDetalleSolicitudOc($codigoSolicitud);
    ...
    function atenderSolicitudCreaOcDiseno() {
        $data['error']    = EXIT_ERROR;
        $data['msj']      = null;
        try {
            $this->db->trans_begin();
            ...
            foreach($arraySolicitud as $row) {
                ...
                if($dataObra['idTipoPlanta'] == ID_TIPO_PLANTA_INTERNA) {
                    ...
                    }else if($dataObra['idProyecto'] == 52)
                    {//FTTH
                        $isFTTH     = true;
                        $objPlanObra['idUsuarioLog'] = $idUsuario;
                        $objPlanObra['fechaLog']    = $fechaActual;
                ...
                array_push($arrayUpdatePlanObraPO, $objPlanObraPO);
            }
            ...
            if ($isFTTH)
            {
                // $data = $this->m_bandeja_solicitud_oc_diseno->updatePlanObraPo($arrayUpdateSolicitud, $arrayUpdateSolicitudxItem, $arrayUpdatePlanObra);
            }
			...
			if(count($disenoList) > 0){
                $reponseInsertBatch = $this->m_bandeja_solicitud_oc_diseno->insertMasiveDiseno($disenoList);
    ...
    function validarSolicitudEdicionOC() {
        ...
        try {
            ...
            $arrayDataObra = $this->m_utils->getDataItemplanByCodOcEdicionDiseno($codigoSolicitud);
			...
			// pre($arrayData);
			// pre($codigoSolicitud);
            $data = $this->m_bandeja_solicitud_oc_diseno->actualizarSolicitud($codigoSolicitud, $arrayData);
            if($data['error'] == EXIT_ERROR) {
                throw new Exception($data['msj']);
            }

            $codigoSolicitudCerti = $this->m_bandeja_solicitud_oc_diseno->getSolCertPndEdicion($arrayDataObra['itemplan']);
            // pre($codigoSolicitudCerti);
            $dataSolCert = array(
                'codigo_solicitud' => $codigoSolicitudCerti,
                'estado'   =>  5//pnd de acta
            );

            $data = $this->m_bandeja_solicitud_oc_diseno->actualizarSolicitud($codigoSolicitudCerti, $dataSolCert);
            if($data['error'] == EXIT_ERROR) {
                throw new Exception($data['msj']);
            }
            $arrayUpdatePO = array(
	...
	function getTablaSolicitudOc($itemplan, $codigoSolicitud) {
        $arrayPlanobra = $this->m_bandeja_solicitud_oc_diseno->getSolicitudOc($itemplan, $codigoSolicitud);
    ...
    function atenderSolicitudCertiOc() {
        ...
        try {
            ...
            $data = $this->m_bandeja_solicitud_oc_diseno->update_solicitud_oc_certi($codigo_solicitud, $arrayData, $dataPlanObra, $idUsuario, $countValida);
