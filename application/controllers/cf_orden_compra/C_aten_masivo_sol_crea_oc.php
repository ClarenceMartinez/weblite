...
class C_aten_masivo_sol_crea_oc extends CI_Controller {
    function __construct() {
        parent::__construct();
        $this->output->set_header('Content-Type: text/html; charset=UTF-8');
        $this->load->model('mf_orden_compra/m_aten_masivo_sol_crea_oc');
    ...
    public function index() {
		...
		//	$data['modulosTopFlotante'] = _getModulosFlotante();
            $this->load->view('vf_orden_compra/v_aten_masivo_sol_crea_oc',$data);        	  
    	 }else{
			...
			foreach ($objectExcel->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					...
					if(strlen($codigoSolicitud) == 0 || $codigoSolicitud == null || $codigoSolicitud == ''){
                        ...
                        $infoSol = $this->m_aten_masivo_sol_crea_oc->getInfoSolicitudOCCreaByCodigo($codigoSolicitud);
                        if($infoSol != null && $infoSol['itemplan'] != null){
                            if(!in_array($ordenCompra,$arrayOC)){
                                if($infoSol['estado'] == 1 && $infoSol['cant'] == 1){
                                    $hasOcActiva = $this->m_aten_masivo_sol_crea_oc->existeItemplanConOC($ordenCompra);
                                    if($hasOcActiva == 0){
                                        $html .= '<tr id="tr' . $count . '" >
                                                    <th style="text-align:center;">
                                                    ' . $count . '
    ...
    public function updateMasivoSolCreacionOC()
    {
        ...
        try {
            ...
            $rutaFinalArchivo = './uploads/aten_masivo_sol_oc_creacion_pan/' . date("Y_m_d_His_").$nombreArchivo;
			...
			if (move_uploaded_file($nombreArchivoTemp, $rutaFinalArchivo)) {
                ...
                if(count($arrayInsertInci) > 0){//para itemplan del nuevo flujo integral(que pasaran a diseño)
                    $data = $this->m_bandeja_solicitud_oc->registroMasivoIncidenciaCV($arrayInsertInci);
                    if($data['error'] == EXIT_ERROR){
                        throw new Exception($data['msj']);
                    }
                }*/
