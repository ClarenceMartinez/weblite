...
class C_aten_masivo_sol_acta_pdt_oc extends CI_Controller {
    function __construct() {
        parent::__construct();
        $this->output->set_header('Content-Type: text/html; charset=UTF-8');
        $this->load->model('mf_orden_compra/m_aten_masivo_sol_acta_pdt_oc');
    ...
    public function index() {
		...
		//	$data['modulosTopFlotante'] = _getModulosFlotante();
            $this->load->view('vf_orden_compra/v_aten_masivo_sol_acta_pdt_oc',$data);        	  
    	 }else{
			...
			foreach ($objectExcel->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					...
					if(strlen($codigoSolicitud) == 0 || $codigoSolicitud == null || $codigoSolicitud == ''){
                        ...
                        $infoSol = $this->m_aten_masivo_sol_acta_pdt_oc->getInfoSolicitudOCCreaByCodigo($codigoSolicitud);                        
    ...
    public function updateMasivoSolCreacionOC()
    {
        ...
        try {
			...
			if($arrayDataFile == null || count($arrayDataFile) == 0){
                ...
                $data = $this->m_aten_masivo_sol_acta_pdt_oc->atencionSolicitudOcEdi($arrayUpdateSolicitud, $arrayUpdatePlanObra);
