...
class C_aten_masivo_sol_acta_pdt_oc_diseno extends CI_Controller {
    function __construct() {
        parent::__construct();
        $this->output->set_header('Content-Type: text/html; charset=UTF-8');
        $this->load->model('mf_orden_compra/m_aten_masivo_sol_acta_pdt_oc_diseno');
    ...
    public function index() {
	    ...
	    if($idUsuario != null){
            ...
            $result = $this->lib_utils->getHTMLPermisos($permisos, ID_ORDEN_COMPRA_PADRE, null, 59, null);
            $data['opciones'] = $result['html'];
            $data['header'] = $this->lib_utils->getHeader();
		//	$data['modulosTopFlotante'] = _getModulosFlotante();
            $this->load->view('vf_orden_compra/v_aten_masivo_sol_acta_pdt_oc_diseno',$data);        	  
    	 }else{
    ...
    public function getFormatoExcelCarga() {
        ...
        try {
            ...
            $data['nombreArchivo'] = 'Formato_Atencion_Masiva_Sol_OC_PdtActa' . date("YmdHis") . '.xls';
    ...
    public function procesarFileMasivoAtenSolCreaOc()
    {
        ...
        try {
            ...
            if (!file_exists("./uploads/aten_masivo_sol_oc_pdtActa_pan")) {
                if (!mkdir("./uploads/aten_masivo_sol_oc_pdtActa_pan")) {
                    throw new Exception('Hubo un error al crear la carpeta aten_masivo_sol_oc_pdtActa_pan!!');
                }
            }
			...
			foreach ($objectExcel->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					...
					if(strlen($codigoSolicitud) == 0 || $codigoSolicitud == null || $codigoSolicitud == ''){
                        ...
                        $infoSol = $this->m_aten_masivo_sol_acta_pdt_oc_diseno->getInfoSolicitudOCCreaByCodigo($codigoSolicitud);                        
                        if($infoSol != null && $infoSol['itemplan'] != null){
                            ...
                            if(!in_array($codigoSolicitud,$arrayOC)){
                                if($infoSol['estado'] == 5 && $infoSol['cant'] == 1){                                  
                                        $html .= '<tr id="tr' . $count . '" >
                                                    <th style="text-align:center;">
                                                    ' . $count . '
                                                    </th>
                                    ...
                                    }else if($infoSol['estado'] == 1){
                                        $msj = 'SOLICITUD PENDIENTE';
                                    }else if($infoSol['estado'] == 2){
                                        $msj = 'SOLICITUD ATENDIDA';
                                    }else if($infoSol['estado'] == 3){
                                        $msj = 'SOLICITUD CANCELADA';
                                    }else if($infoSol['estado'] == 4){
                                        $msj = 'SOLICITUD EN ESPERA DE EDICION';
                                    }
                                    $html .= '<tr style="background-color: #FDBDBD;" id="tr' . $count . '" >
                                                <th style="text-align:center;">
                                                    ' . $count . '
        ...
        try {
			...
			if($arrayDataFile == null || count($arrayDataFile) == 0){
				...
				foreach ($arrayDataFile as $datos) {
                    ...
                    $objSolicitud['estado']            = 1;
                    $objSolicitud['usuario_to_pndte']    = $idUsuario;
                    $objSolicitud['fecha_to_pndte']      = $fechaActual;
                    ...
                    $objPlanObra['estado_oc_certi']  = 'PENDIENTE';         
                    $arrayUpdatePlanObra[] = $objPlanObra;
				}

                $data = $this->m_aten_masivo_sol_acta_pdt_oc_diseno->atencionSolicitudOcEdi($arrayUpdateSolicitud, $arrayUpdatePlanObra);
