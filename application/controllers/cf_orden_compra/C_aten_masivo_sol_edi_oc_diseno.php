...
class C_aten_masivo_sol_edi_oc_diseno extends CI_Controller {
    function __construct() {
        parent::__construct();
        $this->output->set_header('Content-Type: text/html; charset=UTF-8');
        $this->load->model('mf_orden_compra/m_aten_masivo_sol_edi_oc_diseno');
    ...
    public function index() {
		...
		//	$data['modulosTopFlotante'] = _getModulosFlotante();
            $this->load->view('vf_orden_compra/v_aten_masivo_sol_edi_oc_diseno',$data);        	  
    	 }else{
            ...
            foreach ($objectExcel->getWorksheetIterator() as $worksheet) {
                ...
                for ($row = 2; $row <= $highestRow; $row++) {
                    ...
                    if(strlen($codigoSolicitud) == 0 || $codigoSolicitud == null || $codigoSolicitud == ''){
                        ...
                        $infoSol = $this->m_aten_masivo_sol_edi_oc_diseno->getInfoSolicitudOCCreaByCodigo($codigoSolicitud);        
                        // pre($infoSol);              
        ...
        try {
            ...
            if($arrayDataFile == null || count($arrayDataFile) == 0){
                ...
                foreach ($arrayDataFile as $datos) {
                    ...
                    //SOL CERTIFICACION
                    $codigoSolicitudCerti = $this->m_bandeja_solicitud_oc_diseno->getSolCertPndEdicion($itemplan);
                    ...
                    $objPlanObra['estado_oc_dev_diseno']       = 'ATENDIDO';
                    $objPlanObra['costo_unitario_mo']   = $costo_sap;
                    if($codigoSolicitudCerti != null){
                        $objPlanObra['estado_oc_certi_diseno']   = 'EN ESPERA DE ACTA';
                    }                 
                    $arrayUpdatePlanObra[] = $objPlanObra;
                }

                $data = $this->m_aten_masivo_sol_edi_oc_diseno->atencionSolicitudOcEdi($arrayUpdateSolicitud, $arrayUpdatePlanObra);
