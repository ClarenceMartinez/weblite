...
class C_aten_masivo_sol_certi_oc extends CI_Controller {
    function __construct() {
        parent::__construct();
        $this->output->set_header('Content-Type: text/html; charset=UTF-8');
        $this->load->model('mf_orden_compra/m_aten_masivo_sol_certi_oc');
    ...
    public function index() {
		...
		//	$data['modulosTopFlotante'] = _getModulosFlotante();
            $this->load->view('vf_orden_compra/v_aten_masivo_sol_certi_oc',$data);        	  
    	 }else{
    ...
    public function getFormatoExcelCarga() {
        ...
        try {
            ...
            $data['nombreArchivo'] = 'Formato_Atencion_Masiva_Sol_OC_Certi' . date("YmdHis") . '.xls';
			...
			foreach ($objectExcel->getWorksheetIterator() as $worksheet) {
				...
				for ($row = 2; $row <= $highestRow; $row++) {
					...
					if(strlen($codigoSolicitud) == 0 || $codigoSolicitud == null || $codigoSolicitud == ''){
                        ...
                        $infoSol = $this->m_aten_masivo_sol_certi_oc->getInfoSolicitudOCCreaByCodigo($codigoSolicitud);                   
        ...
        try {
			...
			if($arrayDataFile == null || count($arrayDataFile) == 0){
				...
				foreach ($arrayDataFile as $datos) {
                    ...
                    $objPlanObra['solicitud_oc_certi']  = $codigoSolicitud;
                    $objPlanObra['estado_oc_certi']     = 'ATENDIDO';
                    $objPlanObra['fecha_certifica']     = $fechaActual;
                    $objPlanObra['idUsuarioLog']        = $idUsuario;
                    $objPlanObra['fechaLog']            = $fechaActual;
                    $objPlanObra['descripcion']         = 'CERTIFICACION OC';
                    if($idEstadoPlan == 10) {//TRUNCO 
                        $objPlanObra['trunco_situacion']   = ID_ESTADO_PLAN_CERTIFICADO;
                    } else {//SI NO ES TRUNCO SE CERTIFICA LA OBRA
                        $objPlanObra['idEstadoPlan']   = ID_ESTADO_PLAN_CERTIFICADO;
                    }
                ...
                $data = $this->m_aten_masivo_sol_certi_oc->atencionSolicitudOcEdi($arrayUpdateSolicitud, $arrayUpdatePlanObra, $arraySolicitudes, $idUsuario);
