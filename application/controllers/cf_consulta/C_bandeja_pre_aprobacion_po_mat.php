...
class C_bandeja_pre_aprobacion_po_mat extends CI_Controller {
    function __construct() {
        parent::__construct();
        $this->output->set_header('Content-Type: text/html; charset=UTF-8');
        $this->load->model('mf_utils/m_utils');
        $this->load->model('mf_consulta/m_bandeja_pre_aprobacion_po_mat');
    ...
    public function index() {
	    ...
	    if($idUsuario != null){
            ...
            $result = $this->lib_utils->getHTMLPermisos($permisos, 26, null, 64, null);
            $data['opciones'] = $result['html'];
            $data['header']     = $this->lib_utils->getHeader();
            //$data['tablaAprobacionPo'] = $this->getTablaConsulta(null, null);
            $data['json_bandeja'] = $this->getArrayPoMat($this->m_bandeja_pre_aprobacion_po_mat->getBandejaAprobPoJson(null,null));
		//	$data['modulosTopFlotante'] = _getModulosFlotante();
            $this->load->view('vf_consulta/v_bandeja_pre_aprobacion_po_mat',$data);        	  
    	 }else{
    ...
    public function getArrayPoMat($listaPoMat){
        ...
        foreach($listaPoMat as $poMat){
            array_push($listaFinal, array('<a class="btn btn-sm btn-outline-success btn-icon btn-inline-block mr-1" aria-expanded="true" title="Ver Lista Materiales" data-itemplan="'.$poMat['itemplan'].'" onclick="verDetallePO(this)" data-codigo_po="'.$poMat['codigo_po'].'" data-id_empresacolab="'.$poMat['idEmpresaColab'].'">
                                                <i class="fal fa-eye"></i>
                                            </a>
                                            <a class="btn btn-sm btn-outline-success btn-icon btn-inline-block mr-1" aria-expanded="true" title="Aprobar PO" data-itemplan="'.$poMat['itemplan'].'" onclick="preAprobarPO(this)" data-codigo_po="'.$poMat['codigo_po'].'" data-id_empresacolab="'.$poMat['idEmpresaColab'].'">
    ...
    function getTablaConsulta($itemplan, $codigoPO) {
        if($itemplan == null && $codigoPO == null){
            ...
            $arrayData = $this->m_bandeja_pre_aprobacion_po_mat->getBandejaAprobPo($itemplan, $codigoPO);
        }
	...
	public function preAprobarPOMat(){
        ...
        try{
            ...
            $arrayInsertLog = array(
                ...
                "controlador" => 'bandeja pre aprobación po mat'
            );
	...
	function getReporteMateriales()
    {
		...
		try {
			...
			$col = 0;
			$row = 2;
			$listaReporte = $this->m_bandeja_pre_aprobacion_po_mat->getDetalleMateriales($codigoPO);
