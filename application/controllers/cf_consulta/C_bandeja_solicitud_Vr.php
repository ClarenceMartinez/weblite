...
class C_bandeja_solicitud_Vr extends CI_Controller {
    ...
    public function index() {
	    ...
	    if($idUsuario != null){
            ...
            $result = $this->lib_utils->getHTMLPermisos($permisos, ID_GESTION_VR_PADRE, null, 28, null);
		...
		//	$data['modulosTopFlotante'] = _getModulosFlotante();
            $this->load->view('vf_consulta/v_bandeja_solicitud_Vr',$data);        	  
    	 }else{
        ...
        if($listaSolVr!=null){
            foreach($listaSolVr as $poMat){ 
                ...
                $actions .= '<a class="btn btn-sm btn-outline-primary btn-icon btn-inline-block mr-1" aria-expanded="true" title="Visualizar Partidas" data-itemplan="'.$poMat['itemplan'].'"
                                onclick="viewDetalleSolVr(this)" data-codigo="'.$poMat['codigoSolVr'].'" data-codigo_po="'.$poMat['codigo_po'].'">
    ...
    function getTablaConsulta($itemplan, $idJefatura = null, $idEmpresaColab = null, $idFase = null, $tipoAtencion = null) {
        ...
        foreach ($arrayData as $row) {
            ...
            $actions .= '<a class="btn btn-sm btn-outline-primary btn-icon btn-inline-block mr-1" aria-expanded="true" title="Visualizar Partidas" data-itemplan="'.$row['itemplan'].'"
                            onclick="viewDetalleSolVr(this)" data-codigo="'.$row['codigoSolVr'].'" data-codigo_po="'.$row['codigo_po'].'">
                            <i class="fal fa-pencil"></i>
                        </a>';
    ...
    function getTablaDetalleSolVr($itemplan,$codigoPO, $codigoSolVr){
        ...
        foreach($arrayDetalleSol as $row){
            ...
            $html .= '  <tr>                            
                            ...
                            <td>'.$row['vale_reserva'].'</td>
                            <td>'
                                    .'<select '.$disabledSel.' id="selAteVr_'.$count.'" name="selAteVr_'.$count.'" data-pos="'.$count.'" data-tpsol="'.$row['idTipoSolicitudVr'].'" class="select2 form-control w-100" onchange="validarDetalle2(this)">
                            ...
                            <td>
                                ...
                                data-codigo="'.$row['codigoSolVr'].'" data-codigo_material="'.$row['codigo_material'].'" data-pos="'.$count.'" onkeyup="setRechazo(this)" >'.$row['comentario'].'</textarea>
                            </td>
                            <!--
                            <td>
                                <div class="demo">
                                    <div class="custom-control custom-checkbox">
                                        ...
                                        data-codigo_material="'.$row['codigo_material'].'" data-pos="'.$count.'" onchange="validarDetalle(this)">
...
/*
            ...
            foreach($arrayDetalleSol as $row) {
                ...
                }else if($row['flg_estado']   ==  3){//RECHAZADO
                    _log('rechazado........');
                } 
             ...
             //_log('UPDATEAR:'.print_r($arrayUpdateDetallePo,true));
             //_log('INSERTAR:'.print_r($arrayInsertDetallePo,true)); 
             //_log(print_r($arrayUpdateDetSolVr,true));
             //_log(print_r($arrayLogSolVr,true));
