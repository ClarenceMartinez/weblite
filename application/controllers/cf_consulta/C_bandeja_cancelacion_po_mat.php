...
class C_bandeja_cancelacion_po_mat extends CI_Controller {
    function __construct() {
        parent::__construct();
        $this->output->set_header('Content-Type: text/html; charset=UTF-8');
        $this->load->model('mf_utils/m_utils');
        $this->load->model('mf_consulta/m_bandeja_cancelacion_po_mat');
    ...
    public function index() {
	    ...
	    if($idUsuario != null){
            ...
            $result = $this->lib_utils->getHTMLPermisos($permisos, 26, null, 52, null);
            $data['opciones'] = $result['html'];
            $data['header']     = $this->lib_utils->getHeader();
            //$data['tablaAprobacionPo'] = $this->getTablaConsulta(null, null);
            $data['json_bandeja'] = $this->getArrayPoMat($this->m_bandeja_cancelacion_po_mat->getBandejaAprobPoJson(null,null));
		//	$data['modulosTopFlotante'] = _getModulosFlotante();
            $this->load->view('vf_consulta/v_bandeja_cancelacion_po_mat',$data);        	  
    	 }else{
    ...
    public function getArrayPoMat($listaPoMat){
        ...
        foreach($listaPoMat as $poMat){
            array_push($listaFinal, array('<a class="btn btn-sm btn-outline-success btn-icon btn-inline-block mr-1" aria-expanded="true" title="Ver Detalle" data-codigo_po="'.$poMat['codigo_po'].'" onclick="verDetallePO(this)" id="btnDet_'.$poMat['codigo_po'].'">
                                                <i class="fal fa-eye"></i>             
                                            </a>
                                            <a class="btn btn-sm btn-outline-success btn-icon btn-inline-block mr-1" aria-expanded="true" title="Aprobar PO" data-itemplan="'.$poMat['itemplan'].'" onclick="cancelarPoMatConfirmacion($(this))" data-codigo_po="'.$poMat['codigo_po'].'" data-itemplan="'.$poMat['itemplan'].'">
                                                <i class="fal fa-times"></i>
                                            </a>',
                                            $poMat['codigo_po'],$poMat['vale_reserva'],$poMat['itemplan'],$poMat['motivoDesc'],$poMat['observacion'],$poMat['usua_reg'], $poMat['fecha_registro'], $poMat['subProyectoDesc'], $poMat['empresaColabDesc'], $poMat['areaDesc'], $poMat['costo_total'], $poMat['estado'] ));
        }
        return $listaFinal;
    }
    ...
    function getTablaConsulta($itemplan, $codigoPO) {
        if($itemplan == null && $codigoPO == null){
            ...
            $arrayData = $this->m_bandeja_cancelacion_po_mat->getBandejaAprobPo($itemplan, $codigoPO);
        }
        ...
        foreach ($arrayData as $row) {         
            $actions = '<button type="button" class="btn btn-primary ml-auto waves-effect waves-themed" data-codigo_po="'.$row['codigo_po'].'" onclick="verDetallePO(this)" id="btnDet_'.$row['codigo_po'].'">
            ...
            if($row['estado_po'] == ID_ESTADO_PO_PRE_APROBADO){
                $actions .= '<a class="btn btn-sm btn-outline-success btn-icon btn-inline-block mr-1" aria-expanded="true" title="Aprobar PO" data-itemplan="'.$row['itemplan'].'"
                                onclick="cancelarPoMatConfirmacion(this)" data-codigo_po="'.$row['codigo_po'].'" data-id_empresacolab="'.$row['idEmpresaColab'].'">
	...
	function getReporteMateriales()
    {
		...
		try {
			...
			$col = 0;
			$row = 2;
			$listaReporte = $this->m_bandeja_cancelacion_po_mat->getDetalleMateriales($codigoPO);
    ...
    function cancelarPoMatConfirm() {
		...
		try {
			...
			if($codigo_po == null || $codigo_po == '') {
				throw new Exception('No cuenta con codigo_po, verificar.');
			}
            ...
            $data = $this->m_bandeja_cancelacion_po_mat->cancelarPoMat($poUpdate, $arrayInsertLog);
            $data['json_bandeja'] = $this->getArrayPoMat($this->m_bandeja_cancelacion_po_mat->getBandejaAprobPoJson(null,null));
		} catch (Exception $e) {
            $data['msj'] = $e->getMessage();
        }
