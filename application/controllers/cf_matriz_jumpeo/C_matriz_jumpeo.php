...
class C_matriz_jumpeo extends CI_Controller {
    ...
    public function getFormatoExcelCarga() {
        ...
        try {
            ...
            $hoja->getStyle('A1:I1')->applyFromArray($estiloTituloColumnas);
        ...
        if ($objectExcel != '')
        {
            ...
            foreach ($objectExcel->getWorksheetIterator() as $worksheet)
            {
                ...
                for ($row = 2; $row <= $highestRow; $row++)
                {
                    ...
                    // $puerto = $worksheet->getCellByColumnAndRow($col, $row)->getValue();
                    // $puerto = _removeEnterYTabs(trim(utf8_encode(utf8_decode($puerto)),'?'));
                    // $col++;
					...
					$dataArray['port']	= '';
             		...
             		{
             			$dataArray['observacion'] .= 'El itemplan ingresado no tiene nodo respectivo<br>';
             		}
